...
hashtable **preprocessing(char *input_file, int L, int K) {
	...
	// Dataset info contains: min_points, max_points, number_of_curves
...
// Function that returns min curve points, max curve points and number of curves in dataset
config *get_config_information(char *conf_file) {
	char line[LINE_LENGTH];
	char *data;

	config *info = (config*) malloc(sizeof(config));
	info->K = 2;
	info->L = 3;
	...
	while(fgets(line, sizeof(line), conf) != NULL) {
		...
		if (!strcmp(data, "number_of_clusters")) {
			// Get k_cluster from config
			data = strtok(NULL,"\n");
			info->k_cluster = atoi(data);
		}
		else if (!strcmp(data, "number_of_grid_curves")) {
			// Get K from config
			data = strtok(NULL,"\n");
			info->K = atoi(data);
		}
		else if (!strcmp(data, "number_of_hash_tables")) {
			// Get L from config
			data = strtok(NULL,"\n");
			info->L = atoi(data);
		}
